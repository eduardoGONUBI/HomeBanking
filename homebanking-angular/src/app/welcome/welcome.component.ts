...
export class WelcomeComponent implements OnInit {
  balance: number | null = null; // Initialize as null to indicate loading
  ...
  ngOnInit() {
    ...
    if (token) {
      ...
      // Make a GET request to fetch the user data, including balance
      this.http.get<any[]>('http://localhost:3000/api/getUserData', { headers }).subscribe(
        (userData) => {
          ...
          if (loggedInUser) {
            this.balance = loggedInUser.balance;
          } else {
